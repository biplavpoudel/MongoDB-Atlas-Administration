<h1>MongoDB Atlas Administration Notebook</h1>
...
- [ATLAS FOUNDATION AND SECURITY](#atlas-foundation-and-security)
  ...
  - [2. Getting Started with Atlas CLI](#2-getting-started-with-atlas-cli)
    ...
    - [2. Create a Local environment for SDLC](#2-create-a-local-environment-for-sdlc)
      - [1. Configuring Dev Dpeloyment](#1-configuring-dev-dpeloyment)
      - [2. Import a database](#2-import-a-database)
      - [3. Run mongosh without interactive shell](#3-run-mongosh-without-interactive-shell)
    - [3. Create a Cloud cluster using AWS/Azure/GCP for Testing](#3-create-a-cloud-cluster-using-awsazuregcp-for-testing)
    - [4. Create a custom profile for automation](#4-create-a-custom-profile-for-automation)
  - [3. Security](#3-security)
    - [1. Atlas User Management using CLI](#1-atlas-user-management-using-cli)
    - [2. Database User Management using CLI](#2-database-user-management-using-cli)
    - [3. Atlas Security Auditing](#3-atlas-security-auditing)
      - [1. Audit Logs](#1-audit-logs)
      - [2. Activity Feed](#2-activity-feed)
    - [4. Encryption in Atlas](#4-encryption-in-atlas)
      - [CSFLE (Client-Side Field Level Encryption):](#csfle-client-side-field-level-encryption)
  - [4. Replication](#4-replication)
    - [1. Replica Set](#1-replica-set)
      - [Replica Set Members](#replica-set-members)
    - [2. Automatic Failover and Election](#2-automatic-failover-and-election)
      - [Voting Memeber](#voting-memeber)
      - [Priority Value](#priority-value)
    - [3. Operation Log (Oplog)](#3-operation-log-oplog)
      - [1. Retrieve the Recent Oplog Entries](#1-retrieve-the-recent-oplog-entries)
      - [2. Retrive Information about Oplog](#2-retrive-information-about-oplog)
    - [4. Read and Write Concerns](#4-read-and-write-concerns)
      - [1. Specify the Write Concern on an Individual Operation](#1-specify-the-write-concern-on-an-individual-operation)
      - [2. Set the Default Read and Write Concerns for all users](#2-set-the-default-read-and-write-concerns-for-all-users)
      - [3. Read Preferences](#3-read-preferences)
  - [5. Deploying Replica Set in MongoDB Deployment](#5-deploying-replica-set-in-mongodb-deployment)
    - [1. Update DHCP Server](#1-update-dhcp-server)
    - [2. Update DNS Server](#2-update-dns-server)
    - [3. Create three Mongod Instances](#3-create-three-mongod-instances)
    - [4. Keyfile Authentication for internal cluster authentication](#4-keyfile-authentication-for-internal-cluster-authentication)
      - [1. Generate the keyfile](#1-generate-the-keyfile)
      - [2. Set permissions and ownership](#2-set-permissions-and-ownership)
      - [3. Copy the key to other VMs](#3-copy-the-key-to-other-vms)
    - [5. Update `mongod.conf` on all three Replica Set members](#5-update-mongodconf-on-all-three-replica-set-members)
    - [6. Initiate the Replica Set](#6-initiate-the-replica-set)
    - [7. Create Admin User](#7-create-admin-user)
    - [8. Login as Admin User](#8-login-as-admin-user)
    - [9. Change Priority of Members](#9-change-priority-of-members)
    - [10. Initiate an Election](#10-initiate-an-election)
  - [6. Reconfigure Replica Set in MongoDB Deployment](#6-reconfigure-replica-set-in-mongodb-deployment)
    - [1. Retrieve the Status of a mongod Instance](#1-retrieve-the-status-of-a-mongod-instance)
    - [2. Add a Member to a Replica Set](#2-add-a-member-to-a-replica-set)
...
- [ATLAS ADMINISTRATION](#atlas-administration)
  - [1. MongoDB Atlas Administration](#1-mongodb-atlas-administration)
    - [1. Cluster Status and Health](#1-cluster-status-and-health)
      ...
      - [2. Retrieve Process Metric](#2-retrieve-process-metric)
    - [2. User and Database Management](#2-user-and-database-management)
    - [3. Logging Basics](#3-logging-basics)
  - [2. MongoDB Logging Basics](#2-mongodb-logging-basics)
    - [1. MongoDB Logs in Atlas](#1-mongodb-logs-in-atlas)
    - [2. MongoDB Logs on Self-Managed Instances](#2-mongodb-logs-on-self-managed-instances)
      - [1. Start Bash shell inside the Container](#1-start-bash-shell-inside-the-container)
      - [2. Check Default Location for the Log File](#2-check-default-location-for-the-log-file)
      - [3. Check Permissions of the Server Log File and Access it](#3-check-permissions-of-the-server-log-file-and-access-it)
      - [4. Capture logs from Container's output stream](#4-capture-logs-from-containers-output-stream)
      - [5. Retrive Log Messages in mongosh](#5-retrive-log-messages-in-mongosh)
      - [6. View local Atlas deployment logs with the Atlas CLI](#6-view-local-atlas-deployment-logs-with-the-atlas-cli)
    - [3. MongoDB Log Events](#3-mongodb-log-events)
      - [1. Sample Log Entry](#1-sample-log-entry)
      - [2. Field Descriptions](#2-field-descriptions)
      - [3. Field Types (Severity)](#3-field-types-severity)
      - [4. Field Types (Component)](#4-field-types-component)
    - [4. MongoDB Server Log Customizations](#4-mongodb-server-log-customizations)
      - [1. Set a slowms Threshold](#1-set-a-slowms-threshold)
      - [2. Find Slow Operations in a Log](#2-find-slow-operations-in-a-log)
      - [3. Verbosity Levels and Viewing Current Log Verbosity Level](#3-verbosity-levels-and-viewing-current-log-verbosity-level)
      - [4. Configure Log Verbosity Levels](#4-configure-log-verbosity-levels)
        - [1. `systemLog.verbosity` settings](#1-systemlogverbosity-settings)
        - [2. `logComponentVerbosity` Parameter](#2-logcomponentverbosity-parameter)
        - [3. `db.setLogLevel()` command for self-managed deployment](#3-dbsetloglevel-command-for-self-managed-deployment)
    - [5. MongoDB Server Log Rotation and Retention](#5-mongodb-server-log-rotation-and-retention)
      - [1. Rotating Logs in self-managed deployment](#1-rotating-logs-in-self-managed-deployment)
      - [2. Rotating Logs Using rename and reopen rotation behaviour](#2-rotating-logs-using-rename-and-reopen-rotation-behaviour)
      - [3. Automating Log Rotation with the logrotate Service](#3-automating-log-rotation-with-the-logrotate-service)
      - [4. Testing the logrotate Configuration](#4-testing-the-logrotate-configuration)
  - [3. MongoDB Database Metrics \& Monitoring](#3-mongodb-database-metrics--monitoring)
    - [1. Core Metrics and Additional Metrics](#1-core-metrics-and-additional-metrics)
    - [2. Monitoring M10+](#2-monitoring-m10)
    - [3. Configuring Alerts in Atlas CLI](#3-configuring-alerts-in-atlas-cli)
      - [1. View our project’s current Alert Settings](#1-view-our-projects-current-alert-settings)
      - [2. Create a New Alert](#2-create-a-new-alert)
      - [3. Update Alert Settings](#3-update-alert-settings)
      - [4. Delete an Alert](#4-delete-an-alert)
    - [4. Responding to Alerts](#4-responding-to-alerts)
      - [1. View Alerts](#1-view-alerts)
      - [2. Acknowledge an Alert](#2-acknowledge-an-alert)
      - [3. Unacknowledge an Alert](#3-unacknowledge-an-alert)
    - [5. Command Line Metrics](#5-command-line-metrics)
      - [1. serverStatus](#1-serverstatus)
      - [2. currentOp](#2-currentop)
      - [3. killOp](#3-killop)
  - [4. Third-Party Integrations with Prometheus, Percona MongoDB Exporter, and Grafana Server](#4-third-party-integrations-with-prometheus-percona-mongodb-exporter-and-grafana-server)
    - [1. Install Prometheus on Debian VM](#1-install-prometheus-on-debian-vm)
    - [2. Install Grafana](#2-install-grafana)
    - [3. Add Prometheus to the Grafana Server](#3-add-prometheus-to-the-grafana-server)
    - [4. Install Percona MongoDB Exporter](#4-install-percona-mongodb-exporter)
    - [5. Create a New User](#5-create-a-new-user)
    - [6. Create a Service for Percona MongoDB Exporter](#6-create-a-service-for-percona-mongodb-exporter)
    - [7. Configure Percona MongoDB Exporter as a Prometheus Target](#7-configure-percona-mongodb-exporter-as-a-prometheus-target)
    - [8. Configure to listen on all interfaces](#8-configure-to-listen-on-all-interfaces)
    - [9. Test Prometheus Server API](#9-test-prometheus-server-api)
  - [5. MongoDB Atlas Backup \& Recovery](#5-mongodb-atlas-backup--recovery)
    - [1. Back Up and Recover an Atlas Free Tier](#1-back-up-and-recover-an-atlas-free-tier)
      - [1. Back Up a Free-Tier Cluster](#1-back-up-a-free-tier-cluster)
      - [2. Restore a Free-Tier Cluster](#2-restore-a-free-tier-cluster)
      - [2. Backup and Recover an Atlas Flex Tier](#2-backup-and-recover-an-atlas-flex-tier)
      - [3. Back Up and Restore an Atlas Dedicated Tier](#3-back-up-and-restore-an-atlas-dedicated-tier)
...
1. To get server host names, we run:
   ...
   ```
2. Now to download the logs with log name: _mongodb.gz_ and host: _atlas-qrz16v-shard-00-00.5azcnau.mongodb.net:27017_ :
   `bash
...
- **Transport Encryption** (Network Encryption) using _TLS_ and Certificate Authority: _Let's Encrypt_
- **Encryption at Rest** using _WiredTiger Encrypted Storage Engine_, and external KMS such as _AWS KMS_, _Azure Key Vault_, _Google Cloud KMS_
- **In-use Encryption** (after loaded into memory) using _CSFLE_
...
- Can have max of 50 members, with a max of 7 _voting members_.
- mongod instances run on servers (nodes).
- Has a primary and multiple secondaries.

#### Replica Set Members

- Only the Primary member receives _Write_ Operations and keeps a rolling records of all the operations in **Oplog**.
- By default, only the Primary handles all _Read_ operations but that can be changed.
- Secondary member replicates the contents of the Primary member by duplicating Primary's _oplog entries_ and applying all the operations to their own datasets.

### 2. Automatic Failover and Election

![Failover](images/failover.svg)

If the Primary node goes down, **Election** is run to determine the new Primary for Replica Set. The secondary member with most votes from the _Voting Members_ is chosen to be the new Primary. The unavailable Primary becomes new Secondary and uses Oplog to catch up.

Entire election process takes few seconds to complete. Election is triggered when:

- Adding new node to replica set.
- Initiating a replica set.
- Performing Replica set maintenance using: _rs.stepDown()_ or _rs.reconfig()_
...
Oplog is a special collection called a **capped collection** that behaves like a _Ring Buffer_, as the oldest entries in Oplog are overwritten once it reches capacity. Each member in a replica set has its own oplog. <br>
**Note:** Each operation in _Oplog_ (operation log) is idempotent.
...
**WARN**: Use spaces not tabs, as YAML explicitly needs spaces. Also, the reason we use fqdn in `net.bindIp` is because according to official documentation: _Starting in MongoDB 5.0, nodes that are only configured with an IP address fail startup validation and do not start._
...
| :------------ | :----------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **t**         | _Datetime_         | Timestamp of the log message in ISO-8601 format.                                                                                                                                                                                                                                                                                              |
| **s**         | _String_           | Short severity code of the log message.                                                                                                                                                                                                                                                                                                       |
| **c**         | _String_           | Full component string for the log message.                                                                                                                                                                                                                                                                                                    |
| **id**        | _Integer_          | Unique identifier for the log statement.                                                                                                                                                                                                                                                                                                      |
| **ctx**       | _String_           | Name of the thread that caused the log statement.                                                                                                                                                                                                                                                                                             |
| **svc**       | _String_           | Name of the service in whose context the log statement was made. Will be `S` for "shard", `R` "router", or `-` for "unknown" or "none".                                                                                                                                                                                                       |
| **msg**       | _String_           | Log output message passed from the server or driver. If necessary, the message is escaped according to the JSON specification.                                                                                                                                                                                                                |
| **attr**      | _Object_           | One or more key-value pairs for additional log attributes. If a log message does not include any additional attributes, the `attr` object is omitted. Attribute values may be referenced by their key name in the `msg` message body, depending on the message. If necessary, the attributes are escaped according to the JSON specification. |
| **tags**      | _Array of strings_ | Strings representing any tags applicable to the log statement. For example, **["startupWarnings"]**. We can view all the messages with a tag of _startupWarnings_ by running `show log startupWarnings` inside mongosh.                                                                                                                       |
| **truncated** | _Object_           | Information about the log message truncation, if applicable. Only included if the log entry contains at least one truncated `attr` attribute.                                                                                                                                                                                                 |
| **size**      | _Object_           | Original size of a log entry if it has been truncated. Only included if the log entry contains at least one truncated `attr` attribute.                                                                                                                                                                                                       |
...
The default value for **slowms** threshold is _100 milliseconds_. To set a custom threshold, we can do one of three things:
...
The profiler level is set to `0` to _disable profiling completely_, set to `1` for _profiling operations that take longer than the threshold_, and set to `2` for _profiling all_ operations.
...
Verbosity affects log entries in the severity categories **Informational and Debug only**. Severity categories above these levels are _always shown_. <br>
We set verbosity levels to a _high value_ to show detailed logging for debugging or development, or to a _low value_ to minimize writes to the log on a vetted production deployment.
...
To configure the default log level for all components, we use the **systemLog.verbosity** setting. To configure the level of specific components, use the **systemLog.component._name_.verbosity** settings in `/etc/mongodb.conf`.
...
**NOTE**: Prometheus can directly scrape from MongoDB Atlas Cloud after configuring _Authentication Credentials_ and _HTTP Service Discovery_ under `Integrations` inside `Project Settings` of Atlas UI.

Now, for this lab, let's start up my debian-based `mongod0` VM (10.0.2.151) in _ServersNAT_ network in KVM.
...
2. Add the host URI and credentials for Percona user:
   ...
   Alternatively, for security, we can set the credentials in **.env** file and modify **EnvironmentFile** key in _/lib/systemd/system/mongodb_exporter.service_ as: `EnvironmentFile=/etc/mongodb_exporter/.env`
...
## 5. MongoDB Atlas Backup & Recovery

### 1. Back Up and Recover an Atlas Free Tier

`mongodump` is a utility that creates a binary export (BSON) of a database's contents. `mongodump` can export data from:
- Standalone deployments
- Replica set
- Sharded cluster
- Flex clusters

`mongodump` dumps following data to a directory or a binary archive file:
- Collection documents, metadata, and options
- Index definitions
- Writes that occur during the export, if run with the mongodump `--oplog` option.

The syntax for `mongodump` is:
```
mongodump <options> <connection-string>
```

Similarly, `mongorestore` utility loads data from either a binary database dump created by `mongodump` or the standard input into a `mongod` or `mongos` instance.

The synatx for `mongorestore` is:
```
mongorestore <options> <connection-string> <directory or file to restore>
```

**NOTE:** These commands are run in an OS shell like `/bin/bash`, not in `mongosh`.

#### 1. Back Up a Free-Tier Cluster
To back up all databases from an M0 cluster into the default `dump/` directory:
```
mongodump --uri mongodb+srv://Cluster35257@test.5azcnau.mongodb.net --oplog
```
The output for `mongodump` was:
```
Enter password for mongo user:

2025-12-16T18:09:11.818+0545	writing sample_supplies.sales to dump/sample_supplies/sales.bson
2025-12-16T18:09:11.819+0545	writing pets.cats to dump/pets/cats.bson
2025-12-16T18:09:12.162+0545	done dumping pets.cats (1 document)
2025-12-16T18:09:13.235+0545	done dumping sample_supplies.sales (5000 documents)
2025-12-16T18:09:19.374+0545	writing captured oplog to 
2025-12-16T18:09:19.427+0545		dumped 1 oplog entry
```
To look at the `dump/` file structure:
```
tree dump
```
which returns:
...
└── sample_supplies
    ...
    └── sales.metadata.json
```
`--oplog` flag creates a `oplog.bson` file that contains oplog entries that occur during the mongodump operation. Without `--oplog`, if there are write operations during the dump operation, the dump will not reflect any of it.
